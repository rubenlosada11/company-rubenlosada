...
section("Filtrado");
console.log(
  "Envíos entregados en España:",
  filterShipments(sampleShipments, { status: "delivered", destinationCountry: "ES" }).map((s) => s.trackingNumber)
);
console.log(
  "Devoluciones pendientes de revisión:",
  filterReturns(sampleReturns, { status: "pending_review" }).map((r) => r.id)
);
console.log(
  "Clientes de moda o electrónica en EE. UU.:",
  filterClients(sampleClients, { country: "US" }).map((c) => c.companyName)
);
console.log("Caso límite — filtrar array vacío:", filterShipments([], { status: "delivered" }));

section("Ordenamiento");
console.log(
  "Envíos por peso ascendente:",
  sortBy(sampleShipments, "weightKg", "asc").map((s) => `${s.trackingNumber} (${s.weightKg}kg)`)
);
console.log(
  "Envíos por coste descendente:",
  sortBy(sampleShipments, "costEur", "desc")
    .slice(0, 3)
...
console.log(
  "Caso límite — búsqueda binaria en array vacío:",
  binarySearchBy([] as Shipment[], "trackingNumber", "TF-US-00001")
);
console.log(
  "Caso límite — array de un solo elemento:",
  binarySearchBy(sortedByTracking.slice(0, 1), "trackingNumber", sortedByTracking[0].trackingNumber)
);
...
console.log(
  "Clientes con contrato a renovar en 90 días (referencia 2026-08-17):",
  clientsNearContractRenewal(sampleClients, new Date("2026-08-17"), 90).map((c) => c.companyName)
);

section("Validaciones");
for (const shipment of sampleShipments) {
  const result = validateShipment(shipment);
  if (!result.valid) console.log(`Envío inválido [${shipment.id}]:`, result.errors);
}
for (const returnRequest of sampleReturns) {
  const result = validateReturnRequest(returnRequest);
  if (!result.valid) console.log(`Devolución inválida [${returnRequest.id}]:`, result.errors);
}
for (const client of sampleClients) {
  const result = validateClient(client);
  if (!result.valid) console.log(`Cliente inválido [${client.id}]:`, result.errors);
}
for (const carrier of sampleCarriers) {
  const result = validateCarrier(carrier);
  if (!result.valid) console.log(`Transportista inválido [${carrier.id}]:`, result.errors);
}
